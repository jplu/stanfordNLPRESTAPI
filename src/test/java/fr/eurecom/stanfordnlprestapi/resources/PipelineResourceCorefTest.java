 ...
 * @author Julien Plu
 */
public class PipelineResourceCorefTest {
  static final Logger LOGGER = LoggerFactory.getLogger(PipelineResourceCorefTest.class);
  @ClassRule
   ...
   * {@link PipelineResource#coref(HttpServletRequest, String, String)} method with content.
   */
  @Test
  public final void testCorefResponseWithContent() {
    ...
    RDFDataMgr.read(fileModel, this.getClass().getResourceAsStream(
        FileSystems.getDefault().getSeparator() + "coref.ttl"), Lang.TURTLE);
    RDFDataMgr.read(testModel, IOUtils.toInputStream(response.readEntity(String.class),
        Charset.forName("UTF-8")), Lang.TURTLE);
    
    Assert.assertTrue("Issue to get the proper full RDF Turtle model of a context for coref",
        fileModel.isIsomorphicWith(testModel));
  }
  
  /**
   * Test the response returned by the
   * {@link PipelineResource#coref(HttpServletRequest, String, String)} method with a JSON
   * violation.
   */
  @Test
  public final void testCorefResponseWithJsonViolation() {
    final Response response = PipelineResourceCorefTest.RESOURCES.getJerseyTest().target(
        "/v4/coref").request("text/turtle;charset=utf-8").post(Entity.entity("{}",
        MediaType.APPLICATION_JSON_TYPE));
  ...
  /**
   * Test the response returned by the
   * {@link PipelineResource#coref(HttpServletRequest, String, String)} method with an invalid
   * JSON.
   */
  @Test
  public final void testCorefResponseWithInvalidJson() {
    final Response response = PipelineResourceCorefTest.RESOURCES.getJerseyTest().target(
