 ...
 * @author Julien Plu
 */
public class PipelineResourceDateTest {
  static final Logger LOGGER = LoggerFactory.getLogger(PipelineResourceDateTest.class);
  @ClassRule
   ...
   * {@link PipelineResource#date(HttpServletRequest, String, String)} method with content.
   */
  @Test
  public final void testDateResponseWithContent() {
    ...
    RDFDataMgr.read(fileModel, this.getClass().getResourceAsStream(
        FileSystems.getDefault().getSeparator() + "date.ttl"), Lang.TURTLE);
    RDFDataMgr.read(testModel, IOUtils.toInputStream(response.readEntity(String.class),
        Charset.forName("UTF-8")), Lang.TURTLE);
    
    Assert.assertTrue("Issue to get the proper full RDF Turtle model of a context for date",
        fileModel.isIsomorphicWith(testModel));
  }
  
  /**
   * Test the response returned by the
   * {@link PipelineResource#date(HttpServletRequest, String, String)}
   * method with a JSON violation.
   */
  @Test
  public final void testDateResponseWithJsonViolation() {
    final Response response = PipelineResourceDateTest.RESOURCES.getJerseyTest().target(
        "/v4/date").request("text/turtle;charset=utf-8").post(Entity.entity("{}",
  ...
  /**
   * Test the response returned by the
   * {@link PipelineResource#date(HttpServletRequest, String, String)} method with an invalid JSON.
   */
  @Test
  public final void testDateResponseWithInvalidJson() {
    final Response response = PipelineResourceDateTest.RESOURCES.getJerseyTest().target(
        "/v4/date").request("text/turtle;charset=utf-8").post(Entity.entity("{\"content\":\"My "
