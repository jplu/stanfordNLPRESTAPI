 ...
 * @author Julien Plu
 */
public class DateCommand<T extends PipelineConfiguration> extends ConfiguredCommand<T> {
  static final Logger LOGGER = LoggerFactory.getLogger(DateCommand.class);
   ...
   */
  public DateCommand() {
    super("date", "Date command on text");
  }
  ...
  protected final void run(final Bootstrap<T> newBootstrap, final Namespace
    ...
    DateCommand.LOGGER.info("Date analysis uses \"{}\" as configuration file",
        newNamespace.getString("file"));
    
    this.pipeline = new StanfordNlp("properties/date_" + newNamespace.getString("lang") + "_"
  ...
  private void process(final Namespace newNamespace, final T newT)
    ...
    DateCommand.LOGGER.info("Date analysis on \"{}\" :", newNamespace.getString("text"));
    
    final String result = this.pipeline.run(text).rdfString(newT.getName(), NlpProcess.DATE,
        "http://127.0.0.1");
    ...
    if (newNamespace.getString("ofile") != null) {
      ...
      DateCommand.LOGGER.info("{}{}", System.lineSeparator(), result);
    }
  }
