 ...
 * @author Julien Plu
 */
public class NumberCommand<T extends PipelineConfiguration> extends ConfiguredCommand<T> {
  static final Logger LOGGER = LoggerFactory.getLogger(NumberCommand.class);
  ...
  public NumberCommand() {
    super("number", "Number command on text");
  }
  ...
  protected final void run(final Bootstrap<T> newBootstrap, final Namespace
    ...
    NumberCommand.LOGGER.info("Number analysis uses \"{}\" as configuration file",
        newNamespace.getString("file"));
    
    this.pipeline = new StanfordNlp("properties/number_" + newNamespace.getString("lang") + "_"
  ...
  private void process(final Namespace newNamespace, final T newT)
    ...
    NumberCommand.LOGGER.info("Number analysis on \"{}\" :", newNamespace.getString("text"));
    
    final String result = this.pipeline.run(text).rdfString(newT.getName(), NlpProcess.NUMBER,
        "http://127.0.0.1");
    ...
    if (newNamespace.getString("ofile") != null) {
      ...
      NumberCommand.LOGGER.info("{}{}", System.lineSeparator(), result);
    }
  }
