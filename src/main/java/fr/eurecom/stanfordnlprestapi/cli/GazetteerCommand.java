 ...
 * @author Julien Plu
 */
public class GazetteerCommand<T extends PipelineConfiguration> extends ConfiguredCommand<T> {
  static final Logger LOGGER = LoggerFactory.getLogger(GazetteerCommand.class);
  private StanfordNlp pipeline;
  
  /**
   * CorefCommand constructor.
   */
  public GazetteerCommand() {
    super("gazetteer", "Gazetteer command on text");
  }
  ...
  protected final void run(final Bootstrap<T> newBootstrap, final Namespace
    ...
    GazetteerCommand.LOGGER.info("Gazetteer analysis uses \"{}\" as configuration file",
        newNamespace.getString("file"));
    
    this.pipeline = new StanfordNlp("properties/gazetteer_" +  newNamespace.getString("lang") + '_'
  ...
  private void process(final Namespace newNamespace, final T newT)
    ...
    if (newNamespace.getString("ofile") != null) {
      ...
      GazetteerCommand.LOGGER.info("{}{}", System.lineSeparator(), result);
    }
  }
  
  @Override
  public final String toString() {
    return "GazetteerCommand{"
