 ...
 * @author Julien Plu
 */
public class CorefCommand<T extends PipelineConfiguration> extends ConfiguredCommand<T> {
  static final Logger LOGGER = LoggerFactory.getLogger(CorefCommand.class);
  private StanfordNlp pipeline;
  
  /**
   * CorefCommand constructor.
   */
  public CorefCommand() {
    super("coref", "Coref command on text");
  }
  ...
  protected final void run(final Bootstrap<T> newBootstrap, final Namespace
    ...
    NerCommand.LOGGER.info("Coref analysis uses \"{}\" as configuration file",
        newNamespace.getString("file"));
    
    this.pipeline = new StanfordNlp("properties/coref_" +  newNamespace.getString("lang") + '_'
  ...
  private void process(final Namespace newNamespace, final T newT)
    ...
    CorefCommand.LOGGER.info("Coref analysis on \"{}\" :", newNamespace.getString("text"));
    
    final String result = this.pipeline.run(text).rdfString(newT.getName(), NlpProcess.COREF,
        "http://127.0.0.1");
    ...
    if (newNamespace.getString("ofile") != null) {
      ...
      CorefCommand.LOGGER.info("{}{}", System.lineSeparator(), result);
    }
  }
